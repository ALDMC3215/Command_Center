...
const geminiLoadingIndicator = document.getElementById(
...
function assetAttemptFinished(name, success = true) {
  assetsAttempted++;
  loadingStatus.textContent = `Loading ${name}... (${assetsAttempted}/${totalAssetsToAttempt})${
    success ? "" : " - FAILED (Using Default)"
  }`;
  if (assetsAttempted >= totalAssetsToAttempt) {
    ...
    if (!loadingOverlay.classList.contains("hidden")) {
      ...
      showMessage(
        "Hyper Globe Ready! Some textures might be placeholders.",
        "success",
        5000
      );
    }
...
function init() {
  ...
  ambientLight = new THREE.AmbientLight(
    ...
    parseFloat(document.getElementById("ambientLightIntensity").value)
  );
  scene.add(ambientLight);

  directionalLight = new THREE.DirectionalLight(
    0xffffff,
    parseFloat(document.getElementById("sunIntensity").value)
  );
  ...
  loadOptionalTexture(
    "https://threejs.org/examples/textures/eso_dark.jpg",
    "Starfield",
    starMaterial
  );
  scene.add(starField);
  ...
  const atmosphereGeometry = new THREE.SphereGeometry(
    EARTH_RADIUS + ATMOSPHERE_THICKNESS,
    128,
    128
  );
  ...
  controls.autoRotateSpeed = parseFloat(
  ...
  if (totalAssetsToAttempt === 0) {
    ...
    loadingTimeout = setTimeout(() => {
      if (!loadingOverlay.classList.contains("hidden")) {
        ...
        clickableGlobeInfo.classList.remove("hidden");
        showMessage(
          "Globe is ready. Some visual assets might be missing due to loading issues.",
          "warning",
          7000
        );
        console.warn(
          "Loading overlay timed out. Some assets may not have loaded."
        );
      }
...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener(
      "input",
      (event) => (ambientLight.intensity = parseFloat(event.target.value))
    );
  document
    .getElementById("sunIntensity")
    .addEventListener(
      "input",
      (event) => (directionalLight.intensity = parseFloat(event.target.value))
    );
  ...
  document
    ...
    .addEventListener("input", (event) => {
      if (controls) controls.autoRotateSpeed = parseFloat(event.target.value);
    });

  document
    .getElementById("toggleRotationButton")
    .addEventListener("click", handleToggleRotation);
  document.getElementById("toggleSunButton").addEventListener("click", () => {
    if (sunMesh) sunMesh.visible = !sunMesh.visible;
    directionalLight.visible = sunMesh.visible;
    showMessage(`Sun visibility ${sunMesh.visible ? "ON" : "OFF"}.`, "info");
  });
  document
    .getElementById("toggleCloudsButton")
    .addEventListener("click", () => {
      if (cloudMesh) cloudMesh.visible = !cloudMesh.visible;
      showMessage(`Clouds ${cloudMesh.visible ? "ON" : "OFF"}.`, "info");
    });
  document
    .getElementById("toggleAtmosphereButton")
    .addEventListener("click", () => {
      if (atmosphereMesh) atmosphereMesh.visible = !atmosphereMesh.visible;
      showMessage(
        `Atmosphere ${atmosphereMesh.visible ? "ON" : "OFF"}.`,
        "info"
      );
    });

  document
    .getElementById("addRandomMarkerButton")
    .addEventListener("click", addRandomMarker);
  document
    .getElementById("resetViewButton")
    .addEventListener("click", resetView);
  getMarkerInfoButton.addEventListener("click", () =>
    fetchGeoInfoFromGemini(clickedEarthCoords || lastMarkerCoords)
  );

  const panelToggleElements = [togglePanelButton, togglePanelHeader];
  panelToggleElements.forEach((el) =>
    el.addEventListener("click", toggleControlsPanel)
  );
...
function handleToggleRotation() {
  ...
  updateRotationButtonText();
  showMessage(
    controls.autoRotate ? "Globe rotation resumed." : "Globe rotation paused.",
    "info"
  );
}
...
function updateRotationButtonText() {
  ...
  if (controls.autoRotate) {
    toggleRotationButton.innerHTML =
...
function toggleControlsPanel() {
  ...
  showMessage(
    isHidden ? "Controls minimized." : "Controls expanded.",
    "info",
    1500
  );
}
...
function onGlobeClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    getMarkerInfoButton.innerHTML =
      '<i class="fas fa-info-circle"></i>Get Info (Clicked Spot)';

    addTemporaryMarker(point);
    showMessage(
      `Location Clicked: Lat ${clickedEarthCoords.lat}, Lon ${clickedEarthCoords.lon}`,
      "info"
    );
  }
}
...
function addRandomMarker() {
  ...
  const position = sphericalToCartesian(
    EARTH_RADIUS + CLOUD_ALTITUDE + markerRadius,
    lat,
    lon
  );
  ...
  getMarkerInfoButton.classList.remove("hidden");
  getMarkerInfoButton.innerHTML =
    '<i class="fas fa-info-circle"></i>Get Info (Random Marker)';

  showMessage(
    `📍 Random Marker Added: Lat ${lastMarkerCoords.lat}, Lon ${lastMarkerCoords.lon}.`,
    "success"
  );
}
...
async function fetchGeoInfoFromGemini(coordsToFetch) {
  if (!coordsToFetch) {
    showMessage(
      "No location selected. Click on the globe or add a random marker.",
      "warning"
    );
    return;
  }
...
function showMessage(message, type = "info", duration = 3000) {
  ...
  if (type === "success")
    messageBox.style.backgroundColor = "#059669"; // Emerald 600
  else if (type === "error")
    messageBox.style.backgroundColor = "#be123c"; // Rose 700
  ...
  if (messageBox.dataset.transformTimeout)
    clearTimeout(parseInt(messageBox.dataset.transformTimeout));
  if (messageBox.dataset.hideTimeout)
    clearTimeout(parseInt(messageBox.dataset.hideTimeout));
  if (messageBox.dataset.removeShowTimeout)
    clearTimeout(parseInt(messageBox.dataset.removeShowTimeout));
